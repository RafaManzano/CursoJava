...
public class MainCP1 {
	...
	public static void main (String[] args) {
		...
		empleados.add(new Empleado("Rafael", "Manzano", "Medina", LocalDate.of(1990, Month.DECEMBER, 1), LocalDate.now(), TipoDpto.CONTABILIDAD,"rafa@java.com", 1500.20, TipoSexo.HOMBRE));
		...
		empleados.add(new Empleado("Alex", "Reyes", "Jimenez", LocalDate.now(), LocalDate.now(), TipoDpto.NODEFINIDO,"alex@java.com", 2011.75, TipoSexo.HOMBRE));
		...
		for(Map.Entry<TipoDpto, Double> entry : sueldoPorDepartamento.entrySet()) {
			empleados.stream()
			.filter(e -> e.getDpto().equals(entry.getKey()) && e.getSalario() >= entry.getValue())
			.forEach(Empleado::pintarEmpleado);
		}
		
		//12. Imprima un mensaje de felicitación para los trabajadores que llevan mas de 5 años en la empresa.
		empleados.stream()
		.filter(e -> ChronoUnit.YEARS.between(e.getFechaAlta(), LocalDate.now()) >= 5)
		.forEach(e -> System.out.println(e.getNombre() + " felicidades, llevas 5 anhos en la empresa"));
